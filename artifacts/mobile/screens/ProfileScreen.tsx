  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <View style={[styles.header, { paddingTop: topPad + 12 }]}>
        <Pressable onPress={() => setScreen("home")} style={[styles.iconBtn, { borderColor: colors.border }]}>
          <Text style={{ fontSize: 18, color: colors.foreground }}>←</Text>
        </Pressable>
        <Text style={[styles.title, { color: colors.foreground }]}>My Profile</Text>
        <Pressable onPress={editing ? handleCancelEdit : () => setEditing(true)} style={[styles.editChip, { borderColor: colors.primary }]}>
          <Text style={{ color: colors.primary, fontSize: 13, fontWeight: "700" }}>{editing ? "Cancel" : "Edit"}</Text>
        </Pressable>
      </View>
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: insets.bottom + 40 }}>
                ...
                <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8, marginTop: 8 }}>
                  {GENDER_OPTIONS.map(g => (
                    <Pressable key={g} onPress={() => setGender(g)}
                      style={[styles.chip, { borderColor: gender === g ? colors.primary : colors.border, backgroundColor: gender === g ? colors.primary + "20" : "transparent" }]}>
                      <Text style={{ color: gender === g ? colors.primary : colors.foreground, fontSize: 13 }}>{g}</Text>
                    </Pressable>
                  ))}
                </View>
              ) : (
            ...
            <View style={{ flexDirection: "row", gap: 12, marginTop: 12 }}>
              {(["en", "hi"] as const).map(l => (
                <Pressable key={l} onPress={() => handleLangSwitch(l)}
                  style={[styles.langBtn, { borderColor: lang === l ? colors.primary : colors.border, backgroundColor: lang === l ? colors.primary + "15" : "transparent" }]}>
                  <Text style={{ color: lang === l ? colors.primary : colors.foreground, fontWeight: "700", fontSize: 14 }}>
                    {l === "en" ? "🇬🇧 English" : "🇮🇳 हिंदी"}
                  </Text>
                </Pressable>
              ))}
        ...
        <Animated.View entering={FadeInDown.delay(300).duration(400)}>
          <Pressable onPress={() => setScreen("dispute_report")}
            style={[styles.listRow, { borderColor: colors.border, marginTop: 16 }]}>
...
function ProfileField({ label, value, onChange, editing, colors, keyboardType }: {
  ...
  return (
    <View>
      <Text style={[{ fontSize: 11, fontWeight: "600", marginBottom: 4, textTransform: "uppercase" as const, letterSpacing: 0.5, color: colors.mutedForeground }]}>{label}</Text>
      {editing ? (
        <TextInput value={value} onChangeText={onChange} keyboardType={keyboardType ?? "default"}
          placeholderTextColor={colors.mutedForeground}
          style={{ borderWidth: 1, borderRadius: 10, paddingHorizontal: 14, paddingVertical: 10, fontSize: 15, borderColor: colors.border, backgroundColor: colors.card, color: colors.foreground }} />
      ) : (
        <Text style={{ fontSize: 15, fontWeight: "500", paddingVertical: 4, color: colors.foreground }}>{value || "Not set"}</Text>
      )}
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  iconBtn: { width: 40, height: 40, borderRadius: 20, borderWidth: 1, alignItems: "center", justifyContent: "center", marginRight: 12 },
  title: { flex: 1, fontSize: 20, fontWeight: "700", fontFamily: "Inter_700Bold" },
  editChip: { paddingHorizontal: 16, paddingVertical: 8, borderRadius: 20, borderWidth: 1.5 },
  ...
  listRow: { flexDirection: "row", alignItems: "center", gap: 12, padding: 16, borderRadius: 14, borderWidth: 1 },
  listRowText: { flex: 1, fontSize: 15, fontWeight: "600" },
  logoutBtn: { padding: 16, borderRadius: 14, borderWidth: 1.5, alignItems: "center" },
});
