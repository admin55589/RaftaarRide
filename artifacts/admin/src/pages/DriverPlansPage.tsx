...
import {
  ...
  Clock, XCircle, AlertTriangle, ChevronDown, Plus,
} from "lucide-react";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-card border border-border rounded-2xl p-6 w-full max-w-sm shadow-2xl">
        ...
        <p className="text-sm text-muted-foreground mb-4">
          ...
          {driver.planEndAt && (
            <span className="block text-xs mt-1">Current end: {formatDate(driver.planEndAt)}</span>
          )}
        </p>
        ...
        <div className="flex gap-2">
          <button
            onClick={onClose}
            className="flex-1 py-2 rounded-lg text-sm font-medium bg-muted/40 text-muted-foreground hover:bg-muted border border-border transition-colors"
          >
            Cancel
          </button>
          <button
...
export function DriverPlansPage() {
  ...
  const filtered = drivers.filter((d) => {
    ...
    const matchFilter = filter === "all" || d.planStatus === filter;
    const matchVehicle = vehicleFilter === "all" || d.vehicleType === vehicleFilter;
    return matchSearch && matchFilter && matchVehicle;
  });
    ...
    <div className="p-6 space-y-5">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-0.5">Sabhi drivers ke subscription plan ki status dekhein</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <StatCard icon={CheckCircle2}  label="Active Plans"  value={stats.active}  color="bg-green-500/10 text-green-400" />
        <StatCard icon={CalendarClock} label="Free Trials"   value={stats.trial}   color="bg-yellow-500/10 text-yellow-400" />
        <StatCard icon={XCircle}       label="Expired"       value={stats.expired} color="bg-red-500/10 text-red-400" />
        <StatCard icon={AlertTriangle} label="No Plan"       value={stats.no_plan} color="bg-muted text-muted-foreground" />
      </div>
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-muted/30">
                  ...
                  <th className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Plan Status</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Type / Billing</th>
                ...
                {filtered.map((d) => (
                  <tr key={d.id} className="hover:bg-muted/20 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      {d.planType ? (
                        <div>
                          <div className="font-medium text-foreground capitalize">{VEHICLE_LABEL[d.planType] ?? d.planType}</div>
                          <div className="text-xs text-muted-foreground capitalize">{d.planBilling ?? "—"}</div>
                        </div>
                      ) : (
                    ...
                    <td className="px-4 py-3 text-xs text-muted-foreground">
                      {d.planEndAt ? (
                        <span className={d.isActive ? "" : "text-red-400"}>
                    ...
                    <td className="px-4 py-3">
                      {d.isActive ? (
                        <span className={`font-semibold text-sm ${
                          d.daysLeft <= 3 ? "text-red-400" : d.daysLeft <= 7 ? "text-yellow-400" : "text-green-400"
                        }`}>
                          {d.daysLeft}d
                        </span>
                      ) : (
                    ...
                    {/* Trial */}
                    <td className="px-4 py-3">
                      <span className={`text-xs font-medium ${d.trialUsed ? "text-muted-foreground" : "text-yellow-400"}`}>
                        {d.trialUsed ? "Used ✓" : "Available"}
                      </span>
                    </td>
                    {/* Actions */}
                    <td className="px-4 py-3 text-right">
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold bg-yellow-500/10 text-yellow-400 border border-yellow-500/20 hover:bg-yellow-500/20 transition-colors"
                      >
        ...
        {!isLoading && filtered.length > 0 && (
          <div className="px-4 py-3 border-t border-border text-xs text-muted-foreground">
            ...
            {filter !== "all" || vehicleFilter !== "all" ? ` (filtered from ${drivers.length})` : ""}
          </div>
        )}
      </div>
